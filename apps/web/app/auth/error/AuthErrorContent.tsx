...
/**
 * @module apps/web/app/auth/error/AuthErrorContent
 *
 * Client component that reads ?error= from the URL and renders the appropriate
 * error message. Must be wrapped in <Suspense> by its parent page so that
 * Next.js can statically pre-render the page shell.
 */

import { useSearchParams } from "next/navigation";
import Link from "next/link";

const ERROR_INFO: Record<string, { title: string; message: string }> = {
  Configuration: {
  ...
  AccessDenied: {
    title: "Access denied",
    message:
      "You don't have permission to sign in. Please contact your teacher or parent.",
  },
  Verification: {
    title: "Link expired",
    message:
...
export default function AuthErrorContent() {
  ...
  const info = (error && ERROR_INFO[error]) || ERROR_INFO["Default"]!;
